...
public class RaceController extends AbstractBehavior<RaceController.Command> {
    ...
    public Receive<Command> createReceive() {
        return this.newReceiveBuilder()
                .onMessage(StartCommand.class, message -> {
                    ...
                    for (int i = 0; i < 10; i++) {
                        ActorRef<Racer.Command> racer = this.getContext().spawn(Racer.create(), "racer" + i);
                        this.currentPositions.put(racer, 0);
                ...
                .onMessage(GetPositionsCommand.class, message -> {
                    for (ActorRef<Racer.Command> racer : currentPositions.keySet()) {
                        racer.tell(new Racer.PositionCommand(this.getContext().getSelf()));
                        this.displayRace();
                    }
                    return Behaviors.same();
                })
                .onMessage(RacerUpdateCommand.class, message -> {
                    this.currentPositions.put(message.getRacer(), message.getPosition());
                    return Behaviors.same();
                })
                .onMessage(RacerFinishedCommand.class, message -> {
                    this.finishingTimes.put(message.getRacer(), System.currentTimeMillis());

                    if (this.finishingTimes.size() == 10) {
                        return this.raceCompleteMessageHandler();
                    }
                    else {
                        return Behaviors.same();
                    }
    ...
    public Receive<RaceController.Command> raceCompleteMessageHandler() {
        return this.newReceiveBuilder()
                .onMessage(GetPositionsCommand.class, message -> {
//                    for (ActorRef<Racer.Command> racer : this.currentPositions.keySet()) {
//                        this.getContext().stop(racer);
//                    }
                    displayResults();
    ...
    private void displayResults() {
        ...
        this.finishingTimes.values().stream().sorted().forEach(it -> {
            for (ActorRef<Racer.Command> key : this.finishingTimes.keySet()) {
                if (this.finishingTimes.get(key) == it) {
